...
const Football = () => {
    ...
    const handleAnswerOptionClick = (isCorrect, index) => {
        ...
        setTimeout(() => {
            ...
            if (nextQuestion < sportsData.football[level].length) {
        ...
        <main>
            ...
            <div className="quiz-container">
                ...
                <div className="question-count">
                    <span>Question {currentQuestion + 1}</span> / {sportsData.football[level].length}
                </div>
                <div className='question-text'>
                    {sportsData.football[level][currentQuestion].question.split('\n').map((line, index) => (
                ...
                {sportsData.football[level][currentQuestion].options.map((option, index) => {
            ...
            <div className="score-section">
                <p>You scored {score} out of {sportsData.football[level].length}</p>
                <div style={{ marginTop: '20px', display: 'flex', gap: '15px', justifyContent: 'center' }}>
                <button onClick={handleRetake}>Retake Test</button>
                {level === 'pro' ? (
                    <button onClick={() => navigate('/sports')}>Choose Another Quiz</button>
                ) : (
