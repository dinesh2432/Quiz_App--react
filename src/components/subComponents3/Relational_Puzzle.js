...
const Relational_Puzzle = () => {
    ...
    const handleAnswerOptionClick = (isCorrect, index) => {
        ...
        setTimeout(() => {
            ...
            if (nextQuestion < puzzleData.relational_puzzle[level].length) {
        ...
        <main>
            ...
            <div className="quiz-container">
                ...
                <div className="question-count">
                    <span>Question {currentQuestion + 1}</span> / {puzzleData.relational_puzzle[level].length}
                </div>
                <div className='question-text'>
                    {puzzleData.relational_puzzle[level][currentQuestion].question.split('\n').map((line, index) => (
                ...
                {puzzleData.relational_puzzle[level][currentQuestion].options.map((option, index) => {
            ...
            <div className="score-section">
                <p>You scored {score} out of {puzzleData.relational_puzzle[level].length}</p>
                <div style={{ marginTop: '20px', display: 'flex', gap: '15px', justifyContent: 'center' }}>
                <button onClick={handleRetake}>Retake Test</button>
                {level === 'pro' ? (
                    <button onClick={() => navigate('/puzzles')}>Choose Another Quiz</button>
                ) : (
...
export default Relational_Puzzle
